	...
	class GridViewDateTime extends GridViewControlBase
	{
		...
		protected function getItemText($dataField, $parameter)
		{
			if($this->ajaxPostBack)
			{
				...
				return "'<input name=\"{$parameter}\" type=\"datetime\" value=\"'.%{$dataField}%.'\" class=\"datetime\" onchange=\"Rum.evalAsync(\'{$uri}/\',\'".$this->escape($params)."\',\'POST\');\" />'";
			}
			else
			{
				return "'<input name=\"{$parameter}\" type=\"datetime\" value=\"'.%{$dataField}%.'\" class=\"datetime\" />'";
			}
		}
		...
		protected function getFooterText($dataField, $parameter)
		{
			if( !$this->footerText )
			{
				return "'<input name=\"{$parameter}\" type=\"datetime\" class=\"datetime\" />'";
			}
			else
			{
